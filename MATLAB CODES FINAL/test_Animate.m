function test_Animate(s_mobility,s_input,time_step,Area_start,Area_end,node_shape,node_color)
    ...
    hold on;
    for nodeIndex = 1:s_mobility.NB_NODES
        vh_node_pos(nodeIndex) = plot(vs_node(nodeIndex).v_x(1),vs_node(nodeIndex).v_y(1),node_shape,'color',node_color);
    end
    ...
    for timeIndex = 1:length(v_t)
        ...
        drawnow;
    xlim([Area_start Area_end])
    ylim([Area_start Area_end])
    end
